...
サークルには「サークルに所属するユーザーの最大数はサークルのオーナーとなるユーザーを含めて30名まで」という不変条件がある。

が、コードに出てくる数字は`29`で、コードに問題はないものの、言葉との齟齬は誤解を招く。

後続の開発者が、間違っているのではないかと誤認してしまうこともあるため、コードは可能な限り言葉との齟齬がないようにすべき。

以下のようにメンバーをカウントするメソッドを追加するとよい。
