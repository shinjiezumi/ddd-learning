...
ただし、ユーザーに関する処理としてどのようなものがあるか表現できなくなってしまう。

そこでパッケージを作ってまとまりを表現すると良い。具体的には、`ApplicationService.Users`というパッケージを作成し、ここに各サービスを配置する。

まとめることで、ユーザーに関わる処理はひとつのパッケージ、ディレクトリにまとまるので探しやすくなる。

また、あくまで凝集度を高めるために分割しているだけで、「ユースケースごとにクラスを分けるべき」ということではない。

そもそも凝集度が絶対の指標ではない。

クラスを構成するインスタンス変数とメソッドには対応関係がありそれが健全であるかを示す凝集度の視点は、コードを整頓する際の一つの指標としてあるくらいの感覚。

# アプリケーションサービスのインターフェース

より柔軟性を担保するためにアプリケーションサービスのインターフェースを用意することがある。

`Client`、`IUserRegisterService`参照

モックを利用することが可能になり、クライアント側とアプリケーションサービス側でそれぞれ開発を進めることが可能になる。

# サービスとは

サービスはクライアントのために何かを行うモノ。値オブジェクトやエンティティは自身のふるまいを持っているが、サービスは自身のためのふるまいを持たない。

したがってサービスはものごとではなく活動や行動であることが多い。ドメインにおける活動をドメインサービス、アプリケーションとして成り立たせる活動をアプリケーションサービスとしている。

アプリケーションはその利用者の問題を解決するために作られる。

たとえばユーザー登録や退会などはアプリケーションを成り立たせるための操作で、これらのふるまいはドメインに存在する概念ではなくアプリケーション固有の振る舞いであり、それらが実装されるのはアプリケーションサービスとなる。

ドメインサービスとアプリケーションサービスは対象となる領域が異なるだけで本質的には同じもの。

## サービスは状態をもたない

サービスは自身のふるまいを変化させる目的で状態を持たない。

ただ状態を一切持っていないということではない。

`UserRegisterService`は`$userRepository`を状態として持っているが、自身のふるまいを変化させる目的の状態ではない。
...
上記例では`sendMail`が直接的にアプリケーションのふるまいを変更してしまい、インスタンスの状態を気にする必要が出てくる。

状態がもたらす複雑さは開発者を混乱させるもので、状態をもたせる以外の方法を考える。